...
def computeKeypointsAndDescriptors(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5,
    ...
    # 构建最开始的图像
    base_image = _generateBaseImage(image, sigma, assumed_blur)

    # 计算高斯金字塔层数
    num_octaves = _computeNumberOfOctaves(base_image.shape)

    # 生成高斯核
    gaussian_kernels = _generateGaussianKernels(sigma, num_intervals)
    gaussian_images = _generateGaussianImages(base_image, num_octaves, gaussian_kernels)
    dog_images = _generateDoGImages(gaussian_images)
    keypoints = _findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width,
                                       eigenvalue_ratio)
    # 移除复杂点
    keypoints = _removeDuplicateKeypoints(keypoints)

    keypoints = _convertKeypointsToInputImageSize(keypoints)
    # 描述符生成
    descriptors = _generateDescriptors(keypoints, gaussian_images)
...
#########################
# 构建不同尺寸的图像
def _generateBaseImage(image, sigma, assumed_blur):
...
def _computeNumberOfOctaves(image_shape):
    """Compute number of octaves in image pyramid as function of base image shape (OpenCV default)
    """
    return int(round(log(min(image_shape)) / log(2) - 1))


def _generateGaussianKernels(sigma, num_intervals):
    """Generate list of gaussian kernels at which to blur the input image. Default values of sigma, intervals, and octaves follow section 3 of Lowe's paper.
    :param sigma:σ,初始sigma
    :param num_intervals: s --- 尺度参数
    """
...
def _generateGaussianImages(image, num_octaves, gaussian_kernels):
...
def _generateDoGImages(gaussian_images):
...
def _findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width, eigenvalue_ratio,
        ...
        for image_index, (first_image, second_image, third_image) in enumerate(
            ...
            for i in range(image_border_width, first_image.shape[0] - image_border_width):
                for j in range(image_border_width, first_image.shape[1] - image_border_width):
                    # 判断离散空间极值点
                    if _isPixelAnExtremum(first_image[i - 1:i + 2, j - 1:j + 2], second_image[i - 1:i + 2, j - 1:j + 2],
                                          third_image[i - 1:i + 2, j - 1:j + 2], threshold):

                        # 关键点定位
                        localization_result = _localizeExtremumViaQuadraticFit(i, j, image_index + 1, octave_index,
                                                                               num_intervals, dog_images_in_octave,
                                                                               sigma, contrast_threshold,
                                                                               image_border_width, eigenvalue_ratio)
                        # 说明该点为特征点
                        if localization_result is not None:
                            keypoint, localized_image_index = localization_result
                            # 获取关键点方向
                            keypoints_with_orientations = _computeKeypointsWithOrientations(keypoint, octave_index,
                                                                                            gaussian_images[
                                                                                                octave_index][
...
def _isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
...
# 特征点提取与去除偏差
def _localizeExtremumViaQuadraticFit(i, j, image_index, octave_index, num_intervals, dog_images_in_octave, sigma,
    ...
    for attempt_index in range(num_attempts_until_convergence):
        ...
        # 计算差分和Hissan矩阵
        gradient = _computeGradientAtCenterPixel(pixel_cube)
        hessian = _computeHessianAtCenterPixel(pixel_cube)
        # 计算偏移量
        # 进行线性拟合
...
def _computeGradientAtCenterPixel(pixel_array):
...
def _computeHessianAtCenterPixel(pixel_array):
...
def _computeKeypointsWithOrientations(keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36,
...
def _compareKeypoints(keypoint1, keypoint2):
...
def _removeDuplicateKeypoints(keypoints):
    """Sort keypoints and remove duplicate keypoints
    """
    if len(keypoints) < 2:
        return keypoints

    keypoints.sort(key=cmp_to_key(_compareKeypoints))
...
def _convertKeypointsToInputImageSize(keypoints):
...
def _generateDescriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3,
    ...
    for keypoint in keypoints:
        octave, layer, scale = _unpackOctave(keypoint)
